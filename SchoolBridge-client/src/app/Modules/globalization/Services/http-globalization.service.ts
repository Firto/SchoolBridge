...
export class HttpGlobalizationService {
    ...
    private _gettingInfo: boolean = false;
    private _infoGetted: Subject<any> = new Subject<any>();
    ...
    {
        this._gettingInfo = true;
        return this._baseService.send<GlobalizationInfo>(this._ser, "info", null, this.createHeaders(lang)).pipe(
            finalize(() => {
                this._infoGetted.next();
                this._gettingInfo = false;
            })
        );
    }

    public updateBaseUpdateId(): Observable<GlobalizationInfo>{
        return this._baseService.send<GlobalizationInfo>(this._ser, "update-baseupdateid", null, this.createHeaders());
    }

    public setString(name: string, str: string): Observable<any> 
    {
        return this._baseService.send<any>(this._ser, "add-or-upd-string", {name: name, string: str}, this.createHeaders());
    }

    public getStrings(strs: string[]): Observable<Record<string, string>>{
        if (this._gettingInfo)
            return this._infoGetted.pipe(take(1), mergeMap(x => this._baseService.send<Record<string, string>>(this._ser, "strings", {strings: strs}, 
            this.createHeaders())));
    
        return this._baseService.send<Record<string, string>>(this._ser, "strings", {strings: strs}, 
            this.createHeaders());
    }

    public addLanguage(abbName: string, fullName: string): Observable<any>{
        return this._baseService.send<any>(this._ser, "add-language", {abbName, fullName});
    }

    public removeLanguage(abbName: string): Observable<any>{
        return this._baseService.get(this._ser, "remove-language", {params: {abbName: abbName}});
    }
    ...
    private createHeaders(lang: string = ""){
        if (lang)
            ...
            return {headers: {"accept-language": this._infoService.info.currentLanguage,
