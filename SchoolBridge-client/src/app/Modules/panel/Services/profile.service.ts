...
export class ProfileService {
  ...
  private _info: BehaviorSubject<ProfileModel> = new BehaviorSubject<
    ProfileModel
  >(null);
  ...
  constructor(
    ...
    if (this.userService.user != null) this.getInfo().subscribe();
  }

  changeLogin(login: string): Observable<any> {
    return this.baseService
      .send<any>(this._ser, "change-login", { login })
      .pipe(
        tap(() => {
