...
export class UsersService {
  ...
  private _toggleMainGetBuffer: Subject<any> = <Subject<any>>(
    new Subject<any>().pipe(debounceTime(100))
  );
  ...
  constructor(
    ...
    this._mainGetThread = <Subject<ShortUserModel>>new Subject<
      ShortUserModel
    >().pipe(
      ...
      mergeMap((x) => {
        ...
        const mustLoad = x.filter(
          (r) =>
            !Object.keys(this._loadedUsers).includes(r.id) ||
            !this._loadedUsers[r.id].user
        );
    ...
    this._userService.userObs.subscribe((x) => {
      if (!x) this._loadedUsers = {};
    });
  }

  public getMany(model: ShortUserModel[]): Observable<UserModel[]> {
    return this._baseService.send<UserModel[]>(this._ser, "getMany", {
      getTokens: model.map((x) => x.getToken.token),
    });
  }

  public getAll(): Observable<UserModel[]>{
    return this._baseService.send<UserModel[]>(this._ser, "getAll");
  }

  public ban(userId: string, reason: string): Observable<UserModel[]>{
    return this._baseService.send<UserModel[]>(this._ser, "ban", {userId, reason});
  }

  public unban(userId: string): Observable<UserModel[]>{
    return this._baseService.send<UserModel[]>(this._ser, "unban", {userId});
  }
  ...
  public getFull(model: UserModel): User {
    return new User(model, this._onlineService);
  }
}
