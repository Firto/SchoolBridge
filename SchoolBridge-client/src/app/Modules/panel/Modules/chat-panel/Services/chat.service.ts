...
export class ChatService {
  ...
  private _chats: BehaviorSubject<Record<string, Chat>> = new BehaviorSubject<
    Record<string, Chat>
  >({});

  public chats: Observable<Chat[]> = this._chats.pipe(
    map((x) =>
      Object.values(x).sort((a: Chat, b: Chat) => {
        if (a.lastMessage && !b.lastMessage) return -1;
        if (b.lastMessage && !a.lastMessage) return 1;
        if (!b.lastMessage && !a.lastMessage) return 0;
  ...
  constructor(
    ...
    this._serverHub.registerOnServerEvent(
      "chatEvent",
      (model: ChatEventModel) => {
        console.log(model);
        switch (model.type) {
          case "newMessage":
            this._chats.value[model.chatId].addNewMessage(
              <MessageModel>model.source
            );
            //this._chats.next(this._chats.value);
            break;
          case "Typing":
            this._chats.value[model.chatId].setTyping(
              (<TypingModel>model.source).typing
            );
            //this._chats.next(this._chats.value);
            break;
        }
    ...
    this._userService.userObs.subscribe((x) => {
      if (!x) this._chats.next({});
    });
  }

  public localNewChat(chat: ChatModel): Chat {
    if (chat.direct === true)
      this._chats.value[chat.id] = new DirectChat(
  ...
  public typing(chatId: string): Observable<void> {
    return this._connService.send("ChatTyping", chatId);
  }

  public getChats(): Observable<ChatModel[]> {
    return this._baseService.send<ChatModel[]>(this._ser, "getChats").pipe(
      tap((x) => {
