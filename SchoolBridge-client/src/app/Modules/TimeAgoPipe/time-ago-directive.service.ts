...
export class TimeAgoDirectiveService {
  private _mainThread: Observable<number> = interval(3000).pipe(
    tap((x) => {
      ...
      Object.keys(this._times).forEach((x) => {
        if (
          this._times[x].lastUpdateTime == null ||
          TimeAgoDirectiveService.getUpdateInterval(
            _curDate,
            this._times[x].time
          ) <
            TimeAgoDirectiveService.getDiffSeconds(
              _curDate,
              this._times[x].lastUpdateTime
            )
        ) {
  ...
  private _onNewString: BehaviorSubject<
  ...
  constructor(private _gbiService: GlobalizationInfoService,
    ...
    this._gbsService.initConstStrings(this._constStrings);
    this._gbiService.infoObs.subscribe((x) => {
      if (x) {
        const dt = new Date();
        const mm = {};
        Object.keys(this._times).forEach(
          (r) =>
            (mm[r] = TimeAgoDirectiveService.getStringName(
  ...
  public static getUpdateInterval(curDate: Date, time: Date): number {
    const seconds: number = TimeAgoDirectiveService.getDiffSeconds(
      curDate,
      time
    );
    if (seconds < 60) return 2;
    // less than 1 min, update every 2 secs
    else if (seconds < 60 * 60) return 5;
    // less than an hour, update every 30 secs
    else if (seconds < 60 * 60 * 24) return 60;
